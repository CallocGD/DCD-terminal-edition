...
def format_bytes(_bytes:float):
    return format_decimal_suffix(_bytes, '%.2f%sB', factor=1024) or 'N/A'
...
class DCDClient:
    def __init__(
        ...
        # we cannot forward a user-agent because it will trigger boomlings's firewall to not let us in
    ...
    def getGJComments(self,page:int,mode:int=0,count:int=20,total:int=0):
        ...
        return self.post("/database/getGJComments21.php",params=params)
    ...
    def post(self,endpoint:str,params:dict):
        ...
        with self.client.stream("POST",endpoint,data=params, timeout=300) as response:
            if response.status_code == 429:
                ...
                # 3. WAF Finds our proxy's ip address to be unfavorable & ip address used has a low reputation on a dnsblacklist (which is bad)  
            ...
            if data == b"-1":
                ...
                raise ServerMisbehaved("Captcha Was Invoked by Geometry Dash's Server Firewall even though server told us <200:OK>")
    ...
    def handle_response(self,response:httpx.Response):
        ...
        while True:
            ...
            now = perf_counter()
            if not chunk:
                Reporter.clean_download_report()
                return read
            speed = calc_speed(start,now,self.bandwith)
            Reporter.download_report(format_bytes(speed) + "     ")
            read += chunk
            start = now
